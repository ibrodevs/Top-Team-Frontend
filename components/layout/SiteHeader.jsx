    ...
    <>
      ...
      >
        <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-5 md:h-20 md:px-10">
          <Link href="/" className="group flex items-center gap-2" aria-label="Top Team KG — на главную">
            ...
            <span className="mt-0.5 hidden text-[10px] uppercase tracking-[0.3em] text-ash md:block">KG</span>
          </Link>
          ...
          <div className="flex items-center gap-3">
            <Link href="/contact" className="accent-glow hidden items-center bg-gold px-5 py-2.5 font-heading text-xs font-semibold uppercase tracking-wide text-ink transition-colors hover:bg-gold-light md:inline-flex">
              Связаться
            </Link>
        ...
        {open && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] bg-ink/98 backdrop-blur-xl lg:hidden">
            ...
            <nav className="flex flex-col px-5 pt-6" aria-label="Мобильное меню">
              ...
              <Link href="/contact" className="mt-8 inline-flex items-center justify-center bg-gold px-8 py-4 font-heading font-semibold uppercase tracking-wide text-ink">
                Написать клубу
              </Link>
            </nav>
          </motion.div>
